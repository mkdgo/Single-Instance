...
"ios"==c.OS?"8":"4"),this.container=b.documentElement,this.session=(this.session=this.getItem(this.options.appID))?JSON.parse(this.session):void 0,!c.hasToken||c.isCompatible&&this.session||(c.hasToken=!1,f()),c.isCompatible){this.session=this.session||y;try{if(!localStorage)throw Error("localStorage is not defined");localStorage.setItem(this.options.appID,JSON.stringify(this.session));c.hasLocalStorage=!0}catch(p){c.hasLocalStorage=!1,this.options.onPrivate&&this.options.onPrivate.call(this)}d=!this.options.validLocation.length;
...
"error");e.failure()}));return e}function d(a){var c,d,e=[],l=new qq.Promise;f.callbacks.processingDroppedFiles();g.dropDisabled(!0);if(1<a.files.length&&!f.allowMultipleItems)f.callbacks.processingDroppedFilesComplete([]),f.callbacks.dropError("tooManyFilesError",""),g.dropDisabled(!1),l.failure();else{h=[];if(qq.isFolderDropSupported(a))for(c=a.items,a=0;a<c.length;a+=1)(d=c[a].webkitGetAsEntry())&&(d.isFile?h.push(c[a].getAsFile()):e.push(b(d).done(function(){e.pop();0===e.length&&l.success()})));
else h=a.files;0===e.length&&l.success()}return l}function c(a){g=new qq.UploadDropZone({element:a,onEnter:function(c){qq(a).addClass(f.classes.dropActive);c.stopPropagation()},onLeaveNotDescendants:function(){qq(a).removeClass(f.classes.dropActive)},onDrop:function(c){f.hideDropZonesBeforeEnter&&qq(a).hide();qq(a).removeClass(f.classes.dropActive);d(c.dataTransfer).done(function(){var a=h;f.callbacks.dropLog("Grabbed "+a.length+" dropped files.");g.dropDisabled(!1);f.callbacks.processingDroppedFilesComplete(a)})}});
...
d+" request for "+a);!h&&l?b.send(qq.obj2url(l,"")):b.send()}function c(a){var c=p[a].xhr;return function(){if(4===c.readyState){var d=p[a].xhr,e=f(),h=!1;b(a);0<=qq.indexOf(m.successfulResponseCodes,d.status)||(h=!0,g(e+" request for "+a+" has failed - response code "+d.status,"error"));m.onComplete(a,d,h)}}}function e(a){var c=p[a].xhr;a=m.customHeaders;c.setRequestHeader("X-Requested-With","XMLHttpRequest");c.setRequestHeader("Cache-Control","no-cache");qq.each(a,function(a,b){c.setRequestHeader(a,
b)})}function f(){return m.demoMode?"GET":m.method}var g,h,l=[],p=[],m={method:"POST",maxConnections:3,customHeaders:{},endpointStore:{},paramsStore:{},successfulResponseCodes:[200],demoMode:!1,cors:{expected:!1,sendCredentials:!1},log:function(){},onSend:function(){},onComplete:function(){},onCancel:function(){}};qq.extend(m,a);g=m.log;h="GET"===f()||"DELETE"===f();return{send:function(a,c){p[a]={addToPath:c};l.push(a)<=m.maxConnections&&d(a)},cancel:function(a){var c=p[a].xhr,d=f();c?(c.onreadystatechange=
...
qq.UploadHandlerForm=function(a,b,d){function c(a){void 0!==r[a]&&(r[a](),delete r[a])}function e(a,b){var d=a.id;A[m[d]]=b;r[d]=qq(a).attach("load",function(){p[d]&&(y("Received iframe load event for CORS upload request (file id "+d+")"),C[d]=setTimeout(function(){var a="No valid message received from loaded iframe for file id "+d;y(a,"error");b({error:a})},1E3))});K.receiveMessage(d,function(a){y("Received the following window message: '"+a+"'");var b=qq.parseJson(a),e=b.uuid;e&&A[e]?(clearTimeout(C[d]),
delete C[d],c(d),a=A[e],delete A[e],K.stopReceivingMessages(d),a(b)):e||y("'"+a+"' does not contain a UUID - ignoring.")})}function f(a,c){l.cors.expected?e(a,c):r[a.id]=qq(a).attach("load",function(){y("Received response for "+a.id);if(a.parentNode){try{if(a.contentDocument&&a.contentDocument.body&&"false"==a.contentDocument.body.innerHTML)return}catch(b){y("Error when attempting to access iframe during handling of upload response ("+b+")","error")}c()}})}function g(a){var c=qq.toElement('<iframe src="javascript:false;" name="'+
a+'" />');c.setAttribute("id",a);c.style.display="none";document.body.appendChild(c);return c}function h(a,c){var b=l.paramsStore.getParams(a),d=qq.toElement('<form method="'+(l.demoMode?"GET":"POST")+'" enctype="multipart/form-data"></form>'),e=l.endpointStore.getEndpoint(a),f=e;b[l.uuidParamName]=m[a];l.paramsInBody?qq.obj2Inputs(b,d):f=qq.obj2url(b,e);d.setAttribute("action",f);d.setAttribute("target",c.name);d.style.display="none";document.body.appendChild(d);return d}var l=a,p=[],m=[],r={},C=
...
l.cors.expected&&(clearTimeout(C[a]),delete C[a],K.stopReceivingMessages(a));if(a=document.getElementById(a))a.setAttribute("src","java"+String.fromCharCode(115)+"cript:false;"),qq(a).remove()},upload:function(a){var d=p[a],e=t.getName(a),m=g(a),r;if(!d)throw Error("file with passed id was not added, or already uploaded or cancelled");l.onUpload(a,this.getName(a));r=h(a,m);r.appendChild(d);f(m,function(d){y("iframe loaded");if(!d){var f;try{var g=m.contentDocument||m.contentWindow.document,h=g.body.innerHTML;
y("converting iframe's innerHTML to JSON");y("innerHTML = "+h);h&&h.match(/^<pre/i)&&(h=g.body.firstChild.firstChild.nodeValue);f=qq.parseJson(h)}catch(p){y("Error when attempting to parse form upload response ("+p+")","error"),f={success:!1}}d=f}c(a);l.cors.expected||qq(m).remove();if(d.success||!l.onAutoRetry(a,e,d))l.onComplete(a,e,d),b(a)});y("Sending upload request for "+a);r.submit();qq(r).remove();return a}}};
qq.UploadHandlerXhr=function(a,b,d){function c(a,c,b){var d=w.getSize(a);a=w.getName(a);c[q.chunking.paramNames.partIndex]=b.part;c[q.chunking.paramNames.partByteOffset]=b.start;c[q.chunking.paramNames.chunkSize]=b.size;c[q.chunking.paramNames.totalParts]=b.count;c[q.totalFileSizeParamName]=d;v&&(c[q.chunking.paramNames.filename]=a)}function e(a,c){var b=q.chunking.partSize,d=w.getSize(a),e=n[a].file||n[a].blobData.blob,g=b*c,b=g+b>=d?d:g+b,d=f(a),h;e.slice?h=e.slice(g,b):e.mozSlice?h=e.mozSlice(g,
b):e.webkitSlice&&(h=e.webkitSlice(g,b));return{part:c,start:g,end:b,count:d,blob:h,size:b-g}}function f(a){a=w.getSize(a);return Math.ceil(a/q.chunking.partSize)}function g(a,c,b,d){var e=new FormData,f=q.demoMode?"GET":"POST",g=q.endpointStore.getEndpoint(d),h=g,l=w.getName(d),m=w.getSize(d),p=n[d].blobData;a[q.uuidParamName]=n[d].uuid;v&&(a[q.totalFileSizeParamName]=m,p&&(a[q.blobs.paramNames.name]=p.name));q.paramsInBody||(v||(a[q.inputName]=l),h=qq.obj2url(a,g));c.open(f,h,!0);q.cors.expected&&
q.cors.sendCredentials&&(c.withCredentials=!0);return v?(q.paramsInBody&&qq.obj2FormData(a,e),e.append(q.inputName,b),e):b}function h(a,c){var b=q.customHeaders,d=n[a].file||n[a].blobData.blob;c.setRequestHeader("X-Requested-With","XMLHttpRequest");c.setRequestHeader("Cache-Control","no-cache");v||(c.setRequestHeader("Content-Type","application/octet-stream"),c.setRequestHeader("X-Mime-Type",d.type));qq.each(b,function(a,b){c.setRequestHeader(a,b)})}function l(a,c,b){var d=w.getName(a),e=w.getSize(a);
n[a].attemptingResume=!1;q.onProgress(a,d,e,e);q.onComplete(a,d,c,b);delete n[a].xhr;F(a)}function p(a){var b=n[a].remainingChunkIdxs[0],d=e(a,b),f=n[a].xhr=new XMLHttpRequest,l=w.getSize(a),m=w.getName(a),p;void 0===n[a].loaded&&(n[a].loaded=0);x&&n[a].file&&y(a,d);f.onreadystatechange=C(a,f);f.upload.onprogress=function(c){if(c.lengthComputable){var d=c.loaded+n[a].loaded;c=c.total;var f=e(a,b);c-=f.size;var g=w.getSize(a),f=f.count,h=c-n[a].initialRequestOverhead;n[a].lastRequestOverhead=c;0===
...
p(a):(x&&K(a),l(a,b,c))}else l(a,b,c)}}}}function y(a,c){var b=w.getUuid(a),d=n[a].loaded,e=n[a].initialRequestOverhead,f=n[a].estTotalRequestsSize,g=A(a);qq.setCookie(g,b+E+c.part+E+d+E+e+E+f,q.resume.cookiesExpireIn)}function K(a){n[a].file&&(a=A(a),qq.deleteCookie(a))}function A(a){var c=w.getName(a);a=w.getSize(a);var b=q.chunking.partSize,c="qqfilechunk"+E+encodeURIComponent(c)+E+a+E+b;void 0!==I&&(c+=E+I);return c}function t(a){var c=n[a].file||n[a].blobData.blob,b=w.getName(a),d,e;n[a].loaded=
0;d=n[a].xhr=new XMLHttpRequest;d.upload.onprogress=function(c){c.lengthComputable&&(n[a].loaded=c.loaded,q.onProgress(a,b,c.loaded,c.total))};d.onreadystatechange=C(a,d);e=q.paramsStore.getParams(a);c=g(e,d,c,a);h(a,d);J("Sending upload request for "+a);d.send(c)}var q=a,F=b,J=d,n=[],E="|",Q=q.chunking.enabled&&qq.supportedFeatures.chunking,x=q.resume.enabled&&Q&&qq.supportedFeatures.resume,I;I=null===q.resume.id||void 0===q.resume.id||qq.isFunction(q.resume.id)||qq.isObject(q.resume.id)?void 0:
...
$(function(){$("select").each(function(){var a=$(this);"all"!=a.val()&&(a.parent().parent().addClass("active"),a.parent().find(".v").css("color","#fff"))});"block"!=$.session.get("count_drafted")&&$("#count_drafted").prev().click();"block"!=$.session.get("count_assigned")&&$("#count_assigned").prev().click();"block"!=$.session.get("count_past")&&$("#count_past").prev().click();"block"!=$.session.get("count_closed")&&$("#count_closed").prev().click();$("select").on("change",function(){var a=$(this);
"all"!=a.val()?(a.parent().css("background","#888"),a.parent().find(".v").css("color","#fff")):(a.parent().css("background","#E0E0E0"),a.parent().find(".v").css("color","#333"))});$(".teacher_select").on("change",function(){var a=$(this);a.prev("span").removeClass("a").addClass("preloader");data=setData("teacher");history.replaceState(null,null,"f1_teacher?="+f1_teacher_id);$.ajax({type:"POST",url:"/f1_teacher/sortable",data:data,dataType:"json",success:function(b){$.each(b.assignments,function(d){$("."+
d).fadeOut(200).html("");""!=b.assignments[d]?($.each(b.assignments[d],function(a,b){$(b).appendTo($("."+d))}),a.prev("span").removeClass("preloader").addClass("a"),$("."+d).fadeIn(200)):a.prev("span").removeClass("preloader").addClass("a")});$.each(b.counters,function(a,b){$("."+a).html("("+b+")");0<b?($("#"+a).removeClass("hidden"),$("."+a+"_title").css("color","#333"),$("."+a+"_img").css("background-image","url('../img/acc_arrows.png')")):($("#"+a).removeClass("hidden").addClass("hidden"),$("."+
a+"_title").css("color","#aaa"),$("."+a+"_img").css("background-image","none"))});setSubjectOptions(b.subjects);setYearOptions(b.years);setClassOptions(b.classes);setStatusOptions(b.status_select)}})});$(".subject_select").on("change",function(){var a=$(this);a.prev("span").removeClass("a").addClass("preloader");data=setData("subject");history.replaceState(null,null,"f1_teacher?="+f1_subject_id);$.ajax({type:"POST",url:"/f1_teacher/sortable",data:data,dataType:"json",success:function(b){$.each(b.assignments,
function(d){$("."+d).fadeOut(200).html("");""!=b.assignments[d]?($.each(b.assignments[d],function(a,b){$(b).appendTo($("."+d))}),a.prev("span").removeClass("preloader").addClass("a"),$("."+d).fadeIn(200)):a.prev("span").removeClass("preloader").addClass("a")});$.each(b.counters,function(a,b){$("."+a).html("("+b+")");0<b?($("#"+a).removeClass("hidden"),$("."+a+"_title").css("color","#333"),$("."+a+"_img").css("background-image","url('../img/acc_arrows.png')")):($("#"+a).removeClass("hidden").addClass("hidden"),
$("."+a+"_title").css("color","#aaa"),$("."+a+"_img").css("background-image","none"))});setTeacherOptions(b.teachers);setYearOptions(b.years);setClassOptions(b.classes);setStatusOptions(b.status_select)}})});$(".subject_year_select").on("change",function(){var a=$(this);a.prev("span").removeClass("a").addClass("preloader");data=setData("year");history.replaceState(null,null,"f1_teacher?="+f1_year);$.ajax({type:"POST",url:"/f1_teacher/sortable",data:data,dataType:"json",success:function(b){$.each(b.assignments,
function(d){$("."+d).fadeOut(200).html("");""!=b.assignments[d]?($.each(b.assignments[d],function(a,b){$(b).appendTo($("."+d))}),a.prev("span").removeClass("preloader").addClass("a"),$("."+d).fadeIn(200)):a.prev("span").removeClass("preloader").addClass("a")});$.each(b.counters,function(a,b){$("."+a).html("("+b+")");0<b?($("#"+a).removeClass("hidden"),$("."+a+"_title").css("color","#333"),$("."+a+"_img").css("background-image","url('../img/acc_arrows.png')")):($("#"+a).removeClass("hidden").addClass("hidden"),
$("."+a+"_title").css("color","#aaa"),$("."+a+"_img").css("background-image","none"))});setTeacherOptions(b.teachers);setSubjectOptions(b.subjects);setClassOptions(b.classes);setStatusOptions(b.status_select)}})});$(".class_select").on("change",function(){var a=$(this);a.prev("span").removeClass("a").addClass("preloader");data=setData("class");history.replaceState(null,null,"f1_teacher?="+f1_class_id);$.ajax({type:"POST",url:"/f1_teacher/sortable",data:data,dataType:"json",success:function(b){$.each(b.assignments,
function(d){$("."+d).fadeOut(200).html("");""!=b.assignments[d]?($.each(b.assignments[d],function(a,b){$(b).appendTo($("."+d))}),a.prev("span").removeClass("preloader").addClass("a"),$("."+d).fadeIn(200)):a.prev("span").removeClass("preloader").addClass("a")});$.each(b.counters,function(a,b){$("."+a).html("("+b+")");0<b?($("#"+a).removeClass("hidden"),$("."+a+"_title").css("color","#333"),$("."+a+"_img").css("background-image","url('../img/acc_arrows.png')")):($("#"+a).removeClass("hidden").addClass("hidden"),
$("."+a+"_title").css("color","#aaa"),$("."+a+"_img").css("background-image","none"))});setTeacherOptions(b.teachers);setSubjectOptions(b.subjects);setYearOptions(b.years);setStatusOptions(b.status_select)}})});$(".status_select").on("change",function(){var a=$(this);a.prev("span").removeClass("a").addClass("preloader");data=setData("status");history.replaceState(null,null,"f1_teacher?="+f1_status);$.ajax({type:"POST",url:"/f1_teacher/sortable",data:data,dataType:"json",success:function(b){$.each(b.assignments,
function(d){$("."+d).fadeOut(200).html("");""!=b.assignments[d]?($.each(b.assignments[d],function(a,b){$(b).appendTo($("."+d))}),a.prev("span").removeClass("preloader").addClass("a"),$("."+d).fadeIn(200)):a.prev("span").removeClass("preloader").addClass("a")});$.each(b.counters,function(a,b){$("."+a).html("("+b+")");0<b?($("#"+a).removeClass("hidden"),$("."+a+"_title").css("color","#333"),$("."+a+"_img").css("background-image","url('../img/acc_arrows.png')")):($("#"+a).removeClass("hidden").addClass("hidden"),
$("."+a+"_title").css("color","#aaa"),$("."+a+"_img").css("background-image","none"))})}})})});
function setData(a){f1_teacher_id=$(".teacher_select").find(":selected").val();f1_subject_id=$(".subject_select").find(":selected").val();f1_year=$(".subject_year_select").find(":selected").val();f1_class_id=$(".class_select").find(":selected").val();f1_status=$(".status_select").find(":selected").val();f1_type=a;f1_css_assigned=$("#count_assigned").css("display");f1_css_draft=$("#count_drafted").css("display");f1_css_past=$("#count_past").css("display");f1_css_closed=$("#count_closed").css("display");
return data={f1_teacher_id:f1_teacher_id,f1_subject_id:f1_subject_id,f1_year:f1_year,f1_class_id:f1_class_id,f1_status:f1_status,f1_type:f1_type}}function setTeacherOptions(a){$(".teacher_select").html(a);$(".teacher_select").parent().find(".v").html($(".teacher_select").find(":selected").text())}function setSubjectOptions(a){$(".subject_select").html(a);$(".subject_select").parent().find(".v").html($(".subject_select").find(":selected").text())}
function setYearOptions(a){$(".subject_year_select").html(a);$(".subject_year_select").parent().find(".v").html($(".subject_year_select").find(":selected").text())}function setClassOptions(a){$(".class_select").html(a);$(".class_select").parent().find(".v").html($(".class_select").find(":selected").text())}
function setStatusOptions(a){$(".status_select").html(a);""!=a?$(".status_select").parent().find(".v").html($(".status_select").find("option:first").text()):$(".status_select").parent().find(".v").html("All")}
function delRequest(a,b,d){$("#popupDel").attr("del_id",a);$("#popupDel").attr("del_title",b);$("#popupDel").attr("del_section",d);$("#popupDelRes > .modal-dialog > .modal-content > .modal-header > .modal-title").html("Delete Assignment?");$("#popupDelRes > .modal-dialog > .modal-content > .modal-body").html('Please confirm you would like to delete this Assignment <span style="color:#e74c3c;text-decoration:underline;">'+b+"</span> ?");$("#popupDelRes").modal("show")}
$("#popupDel").on("click",function(){var a=$("#popupDel").attr("del_id");$("#popupDel").attr("del_title");var b=$("#popupDel").attr("del_section");if(""!=a||void 0!=a)data={id:a},$(".assignm_"+a).parent().parent().parent().parent().parent(),a=$("."+b).html(),a=a.substring(1,a.length-1),$("."+b).html("("+(a-1)+")"),$.ajax({type:"POST",url:"/f1_teacher/deleteAssignment",data:data,dataType:"json",success:function(a){"true"==a.status&&($(".assignm_"+a.id).parent().fadeOut(300).remove(),showFooterMessage({status:"success",
mess:"Your assignment has been deleted successfull.",anim_a:2E3,anim_b:170}))}});$("#popupDelRes").modal("hide")});function copyAssignment(a){$("#popupCopy").attr("assignment_id",a);$("#popupCopyAss > .modal-dialog > .modal-content > .modal-header > .modal-title").html("Copy Assignment?");$("#popupCopyAss > .modal-dialog > .modal-content > .modal-body").html("Please confirm you wish to copy this Homework for another class?");$("#popupCopyAss").modal("show")}
$("#popupCopy").on("click",function(){var a=$("#popupCopy").attr("assignment_id");if(""!=a||void 0!=a)data={assignment_id:a},$.ajax({type:"POST",url:"/f2b_teacher/copyAssignment",data:data,dataType:"json",success:function(a){0==a.status?showFooterMessage({status:"alert",mess:"Your assignment has not been copied successfull. Please try again later.",anim_a:2E3,anim_b:170}):showFooterMessage({status:"success",mess:"A new copy of this homework assignment has been created for you to assign to another class. You will be redirected to edit this assignment in a moment..",
anim_a:2E3,anim_b:170,onFinish:"redirectToMode('/f2c_teacher/index/"+a.assignment_id+"')"})}});$("#popupCopyAss").modal("hide")});function redirectToMode(a){document.location=a}
function showInfo(a){data={assignment_id:a};$.ajax({type:"POST",url:"/f1_teacher/getDetails",data:data,dataType:"json",success:function(a){1==a.success?($(".title_info").html(a.title),$(".intro_info").html(a.intro),$(".assignment_to_info").html(a.assigned_to),$(".set_by_info").html(a.set_by),$(".publish_info").html(a.publish_date),$(".deadline_info").html(a.deadline_date),$(".grade_type_info").html(a.grade_type),$(".submitted_info").html(a.submitted),$(".marked_info").html(a.marked),$(".status_info").html(a.status),
$(".resources_info").html(a.resources)):($("#infoModal h5.text-error").show(),$(".title_info").html(""),$(".intro_info").html(""),$(".assignment_to_info").html(""),$(".set_by_info").html(""),$(".publish_info").html(""),$(".deadline_info").html(""),$(".grade_type_info").html(""),$(".submitted_info").html(""),$(".marked_info").html(""),$(".status_info").html(""),$(".resources_info").html(""));$("#infoModal").modal("show")}})};
