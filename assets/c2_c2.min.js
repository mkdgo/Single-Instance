...
"ios"==b.OS?"8":"4"),this.container=c.documentElement,this.session=(this.session=this.getItem(this.options.appID))?JSON.parse(this.session):void 0,!b.hasToken||b.isCompatible&&this.session||(b.hasToken=!1,f()),b.isCompatible){this.session=this.session||y;try{if(!localStorage)throw Error("localStorage is not defined");localStorage.setItem(this.options.appID,JSON.stringify(this.session));b.hasLocalStorage=!0}catch(n){b.hasLocalStorage=!1,this.options.onPrivate&&this.options.onPrivate.call(this)}d=!this.options.validLocation.length;
...
(function(a,c,d){function b(b,d,e){b=c.createElement(b);return d&&(b.id=D+d),e&&(b.style.cssText=e),a(b)}function e(){return d.innerHeight?d.innerHeight:a(d).height()}function f(a){var c=x.length;a=(G+a)%c;return 0>a?c+a:a}function g(a,c){return Math.round((/%/.test(a)?("x"===c?I.width():e())/100:1)*parseInt(a,10))}function h(a,c){return a.retinaUrl&&1<d.devicePixelRatio?c.replace(a.photoRegex,a.retinaSuffix):c}function k(a){"contains"in t[0]&&!t[0].contains(a.target)&&(a.stopPropagation(),t.focus())}
function n(){var c,b=a.data(H,O);null==b?(l=a.extend({},ha),console&&console.log&&console.log("Error: cboxElement missing settings object")):l=a.extend({},b);for(c in l)a.isFunction(l[c])&&"on"!==c.slice(0,2)&&(l[c]=l[c].call(H));l.rel=l.rel||H.rel||a(H).data("rel")||"nofollow";l.href=l.href||a(H).attr("href");l.title=l.title||H.title;"string"==typeof l.href&&(l.href=a.trim(l.href))}function m(b,d){a(c).trigger(b);L.triggerHandler(b);a.isFunction(d)&&d.call(H)}function r(d){Y||(H=d,n(),x=a(H),G=0,
"nofollow"!==l.rel&&(x=a("."+V).filter(function(){var c,b=a.data(this,O);return b&&(c=a(this).data("rel")||b.rel||this.rel),c===l.rel}),G=x.index(H),-1===G&&(x=x.add(H),G=x.length-1)),A.css({opacity:parseFloat(l.opacity),cursor:l.overlayClose?"pointer":"auto",visibility:"visible"}).show(),ia&&t.add(A).removeClass(ia),l.className&&t.add(A).addClass(l.className),ia=l.className,l.closeButton?aa.html(l.close).appendTo(F):aa.appendTo("<div/>"),P||(P=Z=!0,t.css({visibility:"hidden",display:"block"}),v=
...
a.ctrlKey||(a.preventDefault(),r(this))}return t?(ka||(ka=!0,W.click(function(){B.next()}),X.click(function(){B.prev()}),aa.click(function(){B.close()}),A.click(function(){l.overlayClose&&B.close()}),a(c).bind("keydown."+D,function(a){var c=a.keyCode;P&&l.escKey&&27===c&&(a.preventDefault(),B.close());P&&l.arrowKey&&x[1]&&!a.altKey&&(37===c?(a.preventDefault(),X.click()):39===c&&(a.preventDefault(),W.click()))}),a.isFunction(a.fn.on)?a(c).on("click."+D,"."+V,b):a("."+V).live("click."+D,b)),!0):!1}
...
p(a(e))):l.iframe?p(" "):l.html?p(l.html):l.photo||l.photoRegex.test(e)?(e=h(l,e),u=c.createElement("img"),a(u).addClass(D+"Photo").bind("error",function(){l.title=!1;p(b(z,"Error").html(l.imgError))}).one("load",function(){var c;q===la&&(a.each(["alt","longdesc","aria-describedby"],function(c,b){var d=a(H).attr(b)||a(H).attr("data-"+b);d&&u.setAttribute(b,d)}),l.retinaImage&&1<d.devicePixelRatio&&(u.height/=d.devicePixelRatio,u.width/=d.devicePixelRatio),l.scalePhotos&&(f=function(){u.height-=u.height*
c;u.width-=u.width*c},l.mw&&u.width>l.mw&&(c=(u.width-l.mw)/u.width,f()),l.mh&&u.height>l.mh&&(c=(u.height-l.mh)/u.height,f())),l.h&&(u.style.marginTop=Math.max(l.mh-u.height,0)/2+"px"),x[1]&&(l.loop||x[G+1])&&(u.style.cursor="pointer",u.onclick=function(){B.next()}),u.style.width=u.width+"px",u.style.height=u.height+"px",setTimeout(function(){p(u)},1))}),setTimeout(function(){u.src=e},1)):e&&w.load(e,l.data,function(c,d){q===la&&p("error"===d?b(z,"Error").html(l.xhrError):a(this).contents())})}var A,
...
V=D+"Element",pa=D+"_open",ca=D+"_load",da=D+"_complete",ma=D+"_cleanup",ja=D+"_closed",ba=D+"_purge",L=a("<a/>"),z="div",la=0,oa={},qa=function(){function a(){clearTimeout(g)}function c(){(l.loop||x[G+1])&&(a(),g=setTimeout(B.next,l.slideshowSpeed))}function b(){U.html(l.slideshowStop).unbind(k).one(k,d);L.bind(da,c).bind(ca,a);t.removeClass(h+"off").addClass(h+"on")}function d(){a();L.unbind(da,c).unbind(ca,a);U.html(l.slideshowStart).unbind(k).one(k,function(){B.next();b()});t.removeClass(h+"on").addClass(h+
"off")}function e(){f=!1;U.hide();a();L.unbind(da,c).unbind(ca,a);t.removeClass(h+"off "+h+"on")}var f,g,h=D+"Slideshow_",k="click."+D;return function(){f?l.slideshow||(L.unbind(ma,e),e()):l.slideshow&&x[1]&&(f=!0,L.one(ma,e),l.slideshowAuto?b():d(),U.show())}}();a.colorbox||(a(C),B=a.fn[O]=a[O]=function(c,b){var d=this;if(c=c||{},C(),y()){if(a.isFunction(d))d=a("<a/>"),c.open=!0;else if(!d[0])return d;b&&(c.onComplete=b);d.each(function(){a.data(this,O,a.extend({},a.data(this,O)||ha,c))}).addClass(V);
(a.isFunction(c.open)&&c.open.call(d)||c.open)&&r(d[0])}return d},B.position=function(c,b){function d(){J[0].style.width=Q[0].style.width=F[0].style.width=parseInt(t[0].style.width,10)-N+"px";F[0].style.height=p[0].style.height=E[0].style.height=parseInt(t[0].style.height,10)-M+"px"}var f,h,k,m=0,n=0,r=t.offset();if(I.unbind("resize."+D),t.css({top:-9E4,left:-9E4}),h=I.scrollTop(),k=I.scrollLeft(),l.fixed?(r.top-=h,r.left-=k,t.css({position:"fixed"})):(m=h,n=k,t.css({position:"absolute"})),n+=!1!==
l.right?Math.max(I.width()-l.w-S-N-g(l.right,"x"),0):!1!==l.left?g(l.left,"x"):Math.round(Math.max(I.width()-l.w-S-N,0)/2),m+=!1!==l.bottom?Math.max(e()-l.h-R-M-g(l.bottom,"y"),0):!1!==l.top?g(l.top,"y"):Math.round(Math.max(e()-l.h-R-M,0)/2),t.css({top:r.top,left:r.left,visibility:"visible"}),q[0].style.width=q[0].style.height="9999px",f={width:l.w+S+N,height:l.h+R+M,top:m,left:n},c){var u=0;a.each(f,function(a){return f[a]!==oa[a]?(u=c,void 0):void 0});c=u}oa=f;c||t.css(f);t.dequeue().animate(f,
{duration:c||0,complete:function(){d();Z=!1;q[0].style.width=l.w+S+N+"px";q[0].style.height=l.h+R+M+"px";l.reposition&&setTimeout(function(){I.bind("resize."+D,B.position)},1);b&&b()},step:d})},B.resize=function(a){var c;P&&(a=a||{},a.width&&(l.w=g(a.width,"x")-S-N),a.innerWidth&&(l.w=g(a.innerWidth,"x")),v.css({width:l.w}),a.height&&(l.h=g(a.height,"y")-R-M),a.innerHeight&&(l.h=g(a.innerHeight,"y")),a.innerHeight||a.height||(c=v.scrollTop(),v.css({height:"auto"}),l.h=v.height()),v.css({height:l.h}),
c&&v.scrollTop(c),B.position("none"===l.transition?0:l.speed))},B.prep=function(d){if(P){var e,g="none"===l.transition?0:l.speed;v.empty().remove();v=b(z,"LoadedContent").append(d);v.hide().appendTo(w.show()).css({width:(l.w=l.w||v.width(),l.w=l.mw&&l.mw<l.w?l.mw:l.w,l.w),overflow:l.scrolling?"auto":"hidden"}).css({height:(l.h=l.h||v.height(),l.h=l.mh&&l.mh<l.h?l.mh:l.h,l.h)}).prependTo(F);w.hide();a(u).css({"float":"none"});e=function(){function d(){!1===a.support.opacity&&t[0].style.removeAttribute("filter")}
