...
d):h.webkitSlice&&(g=h.webkitSlice(e,d));return{part:b,start:e,end:d,count:c,blob:g,size:d-e}}function f(a){a=A.getSize(a);return Math.ceil(a/t.chunking.partSize)}function g(a,b,d,c){var h=new FormData,e=t.demoMode?"GET":"POST",g=t.endpointStore.getEndpoint(c),k=g,f=A.getName(c),n=A.getSize(c),m=p[c].blobData;a[t.uuidParamName]=p[c].uuid;z&&(a[t.totalFileSizeParamName]=n,m&&(a[t.blobs.paramNames.name]=m.name));t.paramsInBody||(z||(a[t.inputName]=f),k=qq.obj2url(a,g));b.open(e,k,!0);t.cors.expected&&
...
!d.match(/^_/)&&void 0!==b()[d]};m=function(a){var d=[],c=Array.prototype.slice.call(arguments,1);h(c,d);return b()[a].apply(b(),d)};a.fn.fineUploader=function(h){var e=this,g=arguments,k=[];this.each(function(f,u){c=a(u);if(b()&&n(h)){if(k.push(m.apply(e,g)),1===e.length)return!1}else"object"!==typeof h&&h?a.error("Method "+h+" does not exist on jQuery.fineUploader"):d.apply(e,g)});return 1===k.length?k[0]:1<k.length?k:this}})(jQuery);
...
$(function(){$("ul.sortable").sortable({update:function(a,b){var c=$("ul.sortable").sortable("toArray",{attribute:"id"});$.post("/d4_teacher/saveorder/",{data:JSON.stringify(c),module_id:$("#module_id").val()},function(a,b){},"json")}});$("ul.sortable").disableSelection();bkLib.onDomLoaded(function(){(new nicEditor({buttonList:"bold italic underline left center justify ol ul removeformat forecolor bgcolor link unlink fontSize fontFamily".split(" ")})).panelInstance("module_intro");(new nicEditor({buttonList:"bold italic underline left center justify ol ul removeformat forecolor bgcolor link unlink fontSize fontFamily".split(" ")})).panelInstance("module_objectives");
(new nicEditor({buttonList:"bold italic underline left center justify ol ul removeformat forecolor bgcolor link unlink fontSize fontFamily".split(" ")})).panelInstance("module_teaching_activities");(new nicEditor({buttonList:"bold italic underline left center justify ol ul removeformat forecolor bgcolor link unlink fontSize fontFamily".split(" ")})).panelInstance("module_assessment_opportunities");(new nicEditor({buttonList:"bold italic underline left center justify ol ul removeformat forecolor bgcolor link unlink fontSize fontFamily".split(" ")})).panelInstance("module_notes")})});
...
function addCButton(a){$("#cboxContent").append('<a class="cdownload" href="/df/index/'+a+'" style="font-size: 24px; color: #e74c3c; position: absolute; bottom: 0; right: 0; margin-right: 30px; margin-bottom: -5px;"><span class="fa fa-download"></span></a>')};
