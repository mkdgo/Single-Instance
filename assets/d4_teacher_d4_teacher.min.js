...
n(a):(B&&y(a),h(a,d,b))}else h(a,d,b)}}}}function r(a,b){var d=A.getUuid(a),c=p[a].loaded,h=p[a].initialRequestOverhead,e=p[a].estTotalRequestsSize,g=x(a);qq.setCookie(g,d+F+b.part+F+c+F+h+F+e,t.resume.cookiesExpireIn)}function y(a){p[a].file&&(a=x(a),qq.deleteCookie(a))}function x(a){var b=A.getName(a);a=A.getSize(a);var d=t.chunking.partSize,b="qqfilechunk"+F+encodeURIComponent(b)+F+a+F+d;void 0!==J&&(b+=F+J);return b}function u(a){var b=p[a].file||p[a].blobData.blob,d=A.getName(a),c,h;p[a].loaded=
0;c=p[a].xhr=new XMLHttpRequest;c.upload.onprogress=function(b){b.lengthComputable&&(p[a].loaded=b.loaded,t.onProgress(a,d,b.loaded,b.total))};c.onreadystatechange=v(a,c);h=t.paramsStore.getParams(a);b=g(h,c,b,a);k(a,c);K("Sending upload request for "+a);c.send(b)}var t=a,G=b,K=c,p=[],F="|",Q=t.chunking.enabled&&qq.supportedFeatures.chunking,B=t.resume.enabled&&Q&&qq.supportedFeatures.resume,J;J=null===t.resume.id||void 0===t.resume.id||qq.isFunction(t.resume.id)||qq.isObject(t.resume.id)?void 0:
...
qq.getCookie(x(a));g=A.getName(a);var k,m,l;if(h){h=h.split(F);if(5===h.length){g=h[0];k=parseInt(h[1],10);m=parseInt(h[2],10);l=parseInt(h[3],10);h=parseInt(h[4],10);h={uuid:g,part:k,lastByteSent:m,initialRequestOverhead:l,estTotalRequestsSize:h};break a}K("Ignoring previously stored resume/chunk cookie for "+g+" - old cookie format","warn")}h=void 0}h&&(g=e(a,h.part),!1!==t.onResume(a,c,q(g))&&(d=h.part,p[a].uuid=h.uuid,p[a].loaded=h.lastByteSent,p[a].estTotalRequestsSize=h.estTotalRequestsSize,
p[a].initialRequestOverhead=h.initialRequestOverhead,p[a].attemptingResume=!0,K("Resuming "+c+" at partition index "+d)))}for(c=f(a)-1;c>=d;--c)p[a].remainingChunkIdxs.unshift(c)}n(a)}else u(a)},cancel:function(a){var b=p[a].xhr;t.onCancel(a,this.getName(a));b&&(b.onreadystatechange=null,b.abort());B&&y(a);delete p[a]},getResumableFilesData:function(){var a=[],b=[];return Q&&B?(a=void 0===J?qq.getCookieNames(RegExp("^qqfilechunk\\"+F+".+\\"+F+"\\d+\\"+F+t.chunking.partSize+"=")):qq.getCookieNames(RegExp("^qqfilechunk\\"+
F+".+\\"+F+"\\d+\\"+F+t.chunking.partSize+"\\"+F+J+"=")),qq.each(a,function(a,d){var c=d.split(F),h=qq.getCookie(d).split(F);b.push({name:decodeURIComponent(c[1]),size:c[2],uuid:h[0],partIdx:h[1]})}),b):[]}}};
...
(a.isFunction(b.open)&&b.open.call(d)||b.open)&&q(d[0])}return d},D.position=function(b,c){function d(){K[0].style.width=Q[0].style.width=G[0].style.width=parseInt(u[0].style.width,10)-N+"px";G[0].style.height=p[0].style.height=F[0].style.height=parseInt(u[0].style.height,10)-M+"px"}var h,f,k,n=0,m=0,q=u.offset();if(J.unbind("resize."+E),u.css({top:-9E4,left:-9E4}),f=J.scrollTop(),k=J.scrollLeft(),l.fixed?(q.top-=f,q.left-=k,u.css({position:"fixed"})):(n=f,m=k,u.css({position:"absolute"})),m+=!1!==
...
b&&z.scrollTop(b),D.position("none"===l.transition?0:l.speed))},D.prep=function(c){if(P){var h,g="none"===l.transition?0:l.speed;z.empty().remove();z=d(C,"LoadedContent").append(c);z.hide().appendTo(A.show()).css({width:(l.w=l.w||z.width(),l.w=l.mw&&l.mw<l.w?l.mw:l.w,l.w),overflow:l.scrolling?"auto":"hidden"}).css({height:(l.h=l.h||z.height(),l.h=l.mh&&l.mh<l.h?l.mh:l.h,l.h)}).prependTo(G);A.hide();a(w).css({"float":"none"});h=function(){function c(){!1===a.support.opacity&&u[0].style.removeAttribute("filter")}
...
b.data("original-aria-invalid"):b.attr("aria-invalid"));b.bind("validation.validation",function(c,d){var g=f(b),e=[];return a.each(t,function(c,f){(g||g.length||d&&d.includeEmpty||k.validatorTypes[c].includeEmpty||k.validatorTypes[c].blockSubmit&&d&&d.submitting)&&a.each(f,function(a,d){k.validatorTypes[c].validate(b,g,d)&&e.push(d.message)})}),e});b.bind("getValidators.validation",function(){return t});var G=0;a.each(t,function(a,b){G+=b.length});b.bind("getValidatorCount.validation",function(){return G});
