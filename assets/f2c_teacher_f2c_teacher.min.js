...
n[a].initialRequestOverhead=g.initialRequestOverhead,n[a].attemptingResume=!0,I("Resuming "+c+" at partition index "+d)))}for(c=e(a)-1;c>=d;--c)n[a].remainingChunkIdxs.unshift(c)}q(a)}else u(a)},cancel:function(a){var b=n[a].xhr;r.onCancel(a,this.getName(a));b&&(b.onreadystatechange=null,b.abort());A&&J(a);delete n[a]},getResumableFilesData:function(){var a=[],b=[];return y&&A?(a=void 0===w?qq.getCookieNames(RegExp("^qqfilechunk\\"+z+".+\\"+z+"\\d+\\"+z+r.chunking.partSize+"=")):qq.getCookieNames(RegExp("^qqfilechunk\\"+
...
(function(a,b,c){function d(c,d,e){c=b.createElement(c);return d&&(c.id=H+d),e&&(c.style.cssText=e),a(c)}function f(){return c.innerHeight?c.innerHeight:a(c).height()}function e(a){var b=A.length;a=(L+a)%b;return 0>a?b+a:a}function g(a,b){return Math.round((/%/.test(a)?("x"===b?w.width():f())/100:1)*parseInt(a,10))}function h(a,b){return a.retinaUrl&&1<c.devicePixelRatio?b.replace(a.photoRegex,a.retinaSuffix):b}function l(a){"contains"in u[0]&&!u[0].contains(a.target)&&(a.stopPropagation(),u.focus())}
function q(){var b,c=a.data(M,Q);null==c?(k=a.extend({},ha),console&&console.log&&console.log("Error: cboxElement missing settings object")):k=a.extend({},c);for(b in k)a.isFunction(k[b])&&"on"!==b.slice(0,2)&&(k[b]=k[b].call(M));k.rel=k.rel||M.rel||a(M).data("rel")||"nofollow";k.href=k.href||a(M).attr("href");k.title=k.title||M.title;"string"==typeof k.href&&(k.href=a.trim(k.href))}function m(c,d){a(b).trigger(c);P.triggerHandler(c);a.isFunction(d)&&d.call(M)}function p(c){aa||(M=c,q(),A=a(M),L=
0,"nofollow"!==k.rel&&(A=a("."+X).filter(function(){var b,c=a.data(this,Q);return c&&(b=a(this).data("rel")||c.rel||this.rel),b===k.rel}),L=A.index(M),-1===L&&(A=A.add(M),L=A.length-1)),D.css({opacity:parseFloat(k.opacity),cursor:k.overlayClose?"pointer":"auto",visibility:"visible"}).show(),ia&&u.add(D).removeClass(ia),k.className&&u.add(D).addClass(k.className),ia=k.className,k.closeButton?U.html(k.close).appendTo(x):U.appendTo("<div/>"),R||(R=ba=!0,u.css({visibility:"hidden",display:"block"}),B=
d(F,"LoadedContent","width:0; height:0; overflow:hidden"),x.css({width:"",height:""}).append(B),K=I.height()+y.height()+x.outerHeight(!0)-x.height(),N=n.width()+z.width()+x.outerWidth(!0)-x.width(),S=B.outerHeight(!0),T=B.outerWidth(!0),k.w=g(k.initialWidth,"x"),k.h=g(k.initialHeight,"y"),B.css({width:"",height:k.h}),G.position(),m(pa,k.onOpen),O.add(fa).hide(),u.focus(),k.trapFocus&&b.addEventListener&&(b.addEventListener("focus",l,!0),P.one(ja,function(){b.removeEventListener("focus",l,!0)})),k.returnFocus&&
...
a.ctrlKey||(a.preventDefault(),p(this))}return u?(ka||(ka=!0,Y.click(function(){G.next()}),Z.click(function(){G.prev()}),U.click(function(){G.close()}),D.click(function(){k.overlayClose&&G.close()}),a(b).bind("keydown."+H,function(a){var b=a.keyCode;R&&k.escKey&&27===b&&(a.preventDefault(),G.close());R&&k.arrowKey&&A[1]&&!a.altKey&&(37===b?(a.preventDefault(),Z.click()):39===b&&(a.preventDefault(),Y.click()))}),a.isFunction(a.fn.on)?a(b).on("click."+H,"."+X,c):a("."+X).live("click."+H,c)),!0):!1}
function J(){var e,f,y,w=G.prep,l=++la;ba=!0;C=!1;M=A[L];q();m(ca);m(da,k.onLoad);k.h=k.height?g(k.height,"y")-S-K:k.innerHeight&&g(k.innerHeight,"y");k.w=k.width?g(k.width,"x")-T-N:k.innerWidth&&g(k.innerWidth,"x");k.mw=k.w;k.mh=k.h;k.maxWidth&&(k.mw=g(k.maxWidth,"x")-T-N,k.mw=k.w&&k.w<k.mw?k.w:k.mw);k.maxHeight&&(k.mh=g(k.maxHeight,"y")-S-K,k.mh=k.h&&k.h<k.mh?k.h:k.mh);e=k.href;na=setTimeout(function(){V.show()},100);k.inline?(y=d(F).hide().insertBefore(a(e)[0]),P.one(ca,function(){y.replaceWith(B.children())}),
