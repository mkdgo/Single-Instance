...
for(var l=this.options.validLocation.length;l--;)if(this.options.validLocation[l].test(b.location.href)){d=!0;break}this.getItem("addToHome")&&this.optOut();if(this.session.optedout)this.doLog("Add to homescreen: not displaying callout because user opted out");else if(this.session.added)this.doLog("Add to homescreen: not displaying callout because already added to the homescreen");else if(d)if(c.isStandalone)this.session.added||(this.session.added=!0,this.updateSession(),this.options.onAdd&&c.hasLocalStorage&&
...
b):d.webkitSlice&&(g=d.webkitSlice(h,b));return{part:c,start:h,end:b,count:e,blob:g,size:b-h}}function f(a){a=y.getSize(a);return Math.ceil(a/t.chunking.partSize)}function g(a,c,b,e){var d=new FormData,f=t.demoMode?"GET":"POST",h=t.endpointStore.getEndpoint(e),g=h,l=y.getName(e),m=y.getSize(e),n=q[e].blobData;a[t.uuidParamName]=q[e].uuid;x&&(a[t.totalFileSizeParamName]=m,n&&(a[t.blobs.paramNames.name]=n.name));t.paramsInBody||(x||(a[t.inputName]=l),g=qq.obj2url(a,h));c.open(f,g,!0);t.cors.expected&&
...
q[a].attemptingResume=!1;t.onProgress(a,e,d,d);t.onComplete(a,e,c,b);delete q[a].xhr;F(a)}function p(a){var b=q[a].remainingChunkIdxs[0],d=e(a,b),f=q[a].xhr=new XMLHttpRequest,h=y.getSize(a),n=y.getName(a),p;void 0===q[a].loaded&&(q[a].loaded=0);z&&q[a].file&&w(a,d);f.onreadystatechange=r(a,f);f.upload.onprogress=function(c){if(c.lengthComputable){var d=c.loaded+q[a].loaded;c=c.total;var f=e(a,b);c-=f.size;var h=y.getSize(a),f=f.count,g=c-q[a].initialRequestOverhead;q[a].lastRequestOverhead=c;0===
b?(q[a].lastChunkIdxProgress=0,q[a].initialRequestOverhead=c,q[a].estTotalRequestsSize=h+f*c):q[a].lastChunkIdxProgress!==b&&(q[a].lastChunkIdxProgress=b,q[a].estTotalRequestsSize+=g);t.onProgress(a,n,d,q[a].estTotalRequestsSize)}};t.onUploadChunk(a,n,m(d));p=t.paramsStore.getParams(a);c(a,p,d);q[a].attemptingResume&&(p[t.resume.paramNames.resuming]=!0);p=g(p,f,d.blob,a);l(a,f);J("Sending chunked upload request for item "+a+": bytes "+(d.start+1)+"-"+d.end+" of "+h);f.send(p)}function n(a){J("Server has ordered chunking effort to be restarted on next attempt for item ID "+
...
V=D+"Element",pa=D+"_open",ca=D+"_load",da=D+"_complete",ma=D+"_cleanup",ja=D+"_closed",ba=D+"_purge",L=a("<a/>"),A="div",la=0,oa={},qa=function(){function a(){clearTimeout(h)}function b(){(k.loop||z[G+1])&&(a(),h=setTimeout(C.next,k.slideshowSpeed))}function c(){U.html(k.slideshowStop).unbind(l).one(l,d);L.bind(da,b).bind(ca,a);u.removeClass(g+"off").addClass(g+"on")}function d(){a();L.unbind(da,b).unbind(ca,a);U.html(k.slideshowStart).unbind(l).one(l,function(){C.next();c()});u.removeClass(g+"on").addClass(g+
"off")}function e(){f=!1;U.hide();a();L.unbind(da,b).unbind(ca,a);u.removeClass(g+"off "+g+"on")}var f,h,g=D+"Slideshow_",l="click."+D;return function(){f?k.slideshow||(L.unbind(ma,e),e()):k.slideshow&&z[1]&&(f=!0,L.one(ma,e),k.slideshowAuto?c():d(),U.show())}}();a.colorbox||(a(r),C=a.fn[O]=a[O]=function(b,c){var d=this;if(b=b||{},r(),w()){if(a.isFunction(d))d=a("<a/>"),b.open=!0;else if(!d[0])return d;c&&(b.onComplete=c);d.each(function(){a.data(this,O,a.extend({},a.data(this,O)||ha,b))}).addClass(V);
