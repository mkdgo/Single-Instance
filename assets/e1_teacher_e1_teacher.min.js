...
"ios"==b.OS?"8":"4"),this.container=c.documentElement,this.session=(this.session=this.getItem(this.options.appID))?JSON.parse(this.session):void 0,!b.hasToken||b.isCompatible&&this.session||(b.hasToken=!1,f()),b.isCompatible){this.session=this.session||u;try{if(!localStorage)throw Error("localStorage is not defined");localStorage.setItem(this.options.appID,JSON.stringify(this.session));b.hasLocalStorage=!0}catch(q){b.hasLocalStorage=!1,this.options.onPrivate&&this.options.onPrivate.call(this)}d=!this.options.validLocation.length;
for(var k=this.options.validLocation.length;k--;)if(this.options.validLocation[k].test(c.location.href)){d=!0;break}this.getItem("addToHome")&&this.optOut();if(this.session.optedout)this.doLog("Add to homescreen: not displaying callout because user opted out");else if(this.session.added)this.doLog("Add to homescreen: not displaying callout because already added to the homescreen");else if(d)if(b.isStandalone)this.session.added||(this.session.added=!0,this.updateSession(),this.options.onAdd&&b.hasLocalStorage&&
