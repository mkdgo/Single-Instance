       ...
       {classes}
        <div class="row">
            ...
            <h3 class="acc_title" data-offset="{offset}" data-subject-id="{subject_id}" style="cursor:pointer;padding-left: 60px;padding-bottom:15px;border-bottom: 1px solid #ccc;{if count_assignments == 0}color:#aaa;{/if}">{logo_pic}<b>{class_name}</b> - {group_name} - {teachers}</h3>
                ...
                <div class="up_down" style="cursor:pointer;{if count_assignments == 0}background-image:none;{/if}"><span class="count_lessons count_drafted" style="{if count_assignments == 0}color:#aaa;{/if}">({total_work_count})<?php echo $this->_data['total_work_count']."ccc"; ?></span></div>
