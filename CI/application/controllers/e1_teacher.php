    ...
    function index($subject_id = '', $module_id = '', $lesson_id = '') {
        ...
        if (!empty($content_pages)) {
            foreach ($content_pages as $kay => $val) {
                ...
                $ITEMS[] = Array('resources_label' => $R_label, 'item_id' => $val->id, 'item_type' => 'e2', 'item_controller' => $controller, 'item_type_delete' => "delete", 'item_title' => $val->title, 'item_order' => $val->order, 'item_iconindex' => '2');
            }
        } else {
        ...
        if (!empty($int_assessments)) {
            foreach ($int_assessments as $assessment_key => $assessment) {
                ...
                if (count($questions) == 0)
                    $R_label = 'No Questions';elseif (count($questions) == 1)
                    $R_label = '1 Question';
                else
                    $R_label = count($questions) . ' Questions';
