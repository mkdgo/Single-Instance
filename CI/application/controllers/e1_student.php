    ...
    function index($subject_id = '',$module_id = '', $lesson_id = '') {
		...
		if(!empty($content_pages)){
			foreach ($content_pages as $kay => $val) {
                ...
                $ITEMS[]=Array('resources_label'=>$R_label, 'item_id'=>$val->id, 'item_type'=>"e2", 'item_type_delete'=>"delete", 'item_title'=>$val->title, 'item_order'=>$val->order, 'item_iconindex'=>$icon);
			}
		} else {
