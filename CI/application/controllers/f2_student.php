...
class F2_student extends MY_Controller {
    ...
    function index($id, $mode=1) {
        ...
        if( $assignment->publish_marks == 1 || ( $assignment->grade_type == 'offline' && $assignment->deadline_date < date('Y-m-d') ) ) {
            redirect('/f2_student/marked/'.$assignment->id);
        }
...
//*/
	...
	function marked($id, $mode=1) {
        ...
        if( $assignment->publish_marks != 1  ) {
            if( $assignment->grade_type != 'offline' || $assignment->deadline_date >= date('Y-m-d') ) {
                redirect('/f2_student/index/'.$assignment->id);
            }
        }

        $this->getAssignmentDetails( $assignment );
/*
        $this->_data['set_by'] = $this->user_model->getUserName( $assignment->teacher_id );
        $this->_data['base_assignment_id'] = $assignment->base_assignment_id;
		$this->_data['assignment_id'] = $id;
...
//*/
		...
		if( !empty($resources) ) {
			...
			foreach ($resources as $k => $v) {
                ...
                $this->_data['resources'][$k]['li_style'] = '';
                if( in_array($v->type, $this->_test_resources ) ) {
//echo '<pre>';var_dump( $this->_data['publish_marks']  );die;
                    if( !$this->student_answers_model->isExist( $this->session->userdata('id'), $v->res_id, false, $assignment->id, 'homework' ) && $this->_data['publish_marks'] == 0 ) {
                        $this->_data['resources'][$k]['preview'] = $this->resoucePreview($v, '/f2_student/resource/');
                        $action_required = 'Action Required';
                    } elseif( $this->_data['publish_marks'] == 0 ) {
                        $this->_data['resources'][$k]['preview'] = $this->resoucePreview($v, '/f2a_student/resource/');
                        $action_required = 'Question Answered';
                    } else {
                        $this->_data['resources'][$k]['preview'] = $this->resoucePreview($v, '/f2b_student/resource/');
...
//*/
        ...
        $paste_public = 'f2_student_marked';
        if( $assignment->grade_type == 'test' ) {
            $paste_public = 'f2_student_test_marked';
        }
		$this->_paste_public($paste_public);
	}

    private function getAssignmentDetails( $assignment ) {
        $this->_data['set_by'] = $this->user_model->getUserName( $assignment->teacher_id );
        $this->_data['base_assignment_id'] = $assignment->base_assignment_id;
        $this->_data['assignment_id'] = $assignment->id;
        ...
        if( !$this->checkValidDate($assignment->id) ) { $del_enabled = false; } else { $del_enabled = true; }
        if( !$this->checkValidMarked($assignment->id) ) { $this->_data['marked'] = 1; } else { $this->_data['marked'] = 0; }
