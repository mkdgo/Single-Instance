...
class D4_teacher extends MY_Controller {
    ...
    function index($subject_id, $module_id = '0') {
        ...
        foreach ($this->keyword_model->getKeyword($module_id, 'key_words_objectives', 'key_words_modules', 'module') as $keyword) {
            $module_keywords[] = $keyword->word;
        }

        $this->_data['module_plenary_keywords'] = str_replace('"', "", json_encode($module_keywords));
        $this->_data['module_plenary_keywords_a'] = str_replace('"', "", json_encode($module_keywords));
