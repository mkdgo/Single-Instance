    ...
    function index($subject_id = '', $module_id = '', $lesson_id = '', $page_num = 1, $type = 'view') {
        ...
        foreach ($content_pages as $key => $val) {
            ...
            if (strtolower($val->slide_type) === 'plenary') {
                $contentPagePlenaries = $this->plenary_model->getContentPagePlenaries($val->id);
                if (count($contentPagePlenaries) > 0) {
                    foreach ($contentPagePlenaries as $plenary) {
                        $columnLabels = array();
                        $rows = array();
                        $plenaryGrid = $this->plenary_model->getPlenaryGrid(intval($plenary->plenary_id));
                        if (count($plenaryGrid) === 0) {
                            continue;
                        }

                        foreach ($plenaryGrid as $gridItem) {
                            if (!array_key_exists($gridItem->label_rank, $columnLabels)) {
                                $columnLabels[$gridItem->label_rank] = $gridItem->label;
                            }

                            if (!array_key_exists($gridItem->objective_id, $rows)) {
                                $rows[$gridItem->objective_id] = array(
                                    'label' => $gridItem->objective,
                                    'label_id' => $gridItem->objective_id,
                                    'values' => array()
                                );
                            }

                            $rows[$gridItem->objective_id]['values'][$gridItem->label_rank] = $gridItem->id;
                        }
                        ...
                        ksort($columnLabels);
                        $convertedColumnLabels = array();
                        foreach ($columnLabels as $label) {
                            $convertedColumnLabels[] = array('label_name' => $label);
                        }

                        $plenaries[$plenary->plenary_id] = array(
                            'labels' => $convertedColumnLabels,
                            'rows' => $rows
                        );
                    }
                }
