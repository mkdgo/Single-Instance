...
//echo $this->db->last_query();die;
	...
	public function get_cont_page_resources($cont_page_id = '') {
		$this->db->select(array( 'cont_page_resources.id as slide_res_id', 'resources.id as res_id', 'resources.name', 'resources.type', 'resources.resource_name', 'resources.is_remote', 'resources.link', 'content', 'behavior'));
		$this->db->from($this->_table);
		$this->db->join($this->_cont_page_resources, 'resources.id = cont_page_resources.resource_id');
	...
	public function get_assignment_resources($assignment_id) {
		$this->db->select(array('assignments_resources.is_late AS is_late', 'resources.id AS id', 'resources.id AS res_id', 'resources.name','resources.type', 'resources.resource_name', 'resources.is_remote', 'resources.link', 'content', 'behavior'));
		$this->db->from($this->_table);
...
//echo $this->db->last_query();die;
	...
	public function get_resource_usage($resource_id) {
		...
		$q_modules = $this->db->query("
            SELECT subject_years.year,modules.name as title 
            FROM modules_resources
		...
		$q_lessons = $this->db->query("
            SELECT lessons.title,subject_years.year 
		...
		$q_assignment = $this->db->query("
            SELECT classes.year,assignments.title 
            FROM assignments_resources
		...
		$q_slides = $this->db->query("
            SELECT content_page_slides.title,subject_years.year 
            FROM cont_page_resources
