...
class Pictures_model extends CI_Model {
    ...
    public function save_picture($picture_data = array(), $picture_id = '') 
        {      
        if ($picture_id != '') {            $this->db->where('id', $picture_id);            $this->db->update($this->_table, $picture_data);        } else {            $this->db->insert($this->_table, $picture_data);
