...
class Openid{
  ...
  var $ext_args;
    ...
    function authenticate($openId, $screen_message)
    {
    $consumer = $this->_getConsumer();
        $authRequest = $consumer->begin($openId);

        // No auth request mesettings = new Settings();ans we can't begin OpenID.
    if (!$authRequest)
    {
    ...
    else
    {
        ...
        //change formMarkup to ->htmlMarkup
        //
        ...
        else
        {
            ...
            $screen_message=  str_replace('%FID%', $form_id, $screen_message);
            $screen_message=  str_replace('%FHTML%', $form_html, $screen_message);
