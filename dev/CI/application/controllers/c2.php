...
class C2 extends MY_Controller {
	...
	//	$this->_data['back'] = "/c1/index/{$type}/{$elem_id}" . ($subject_id ? '/' . $subject_id : '') . ($module_id ? '/' . $module_id : '') . ($lesson_id ? '/' . $lesson_id : '') . ($assessment_id ? '/' . $assessment_id : '');
		...
		if (!empty($resource)) {
				...
				$this->_data['resource_keywords'] = json_encode($resource_keywords);
				
                                $this->_data['resource_keywords_a'] = json_encode($resource_keywords);
		...
		/*$classes = $resource->restriction_year; //$this->classes_model->get_classes_for_teacher($this->session->userdata('id'), $subject_id);
 ...
 */
	...
	public function save() {
		...
		if($elem_id > 0) {
			...
			$resource_id = $this->resources_model->save($db_data);
                ...
                $this->keyword_model->updateResourceKeywords( json_decode( $this->input->post('resource_keywords_a') ), $resource_id );
                
		redirect("/c1/save/{$resource_iresource_idd}/{$type}/{$elem_id}" . ($subject_id ? '/' . $subject_id : '') . ($module_id ? '/' . $module_id : '') . ($lesson_id ? '/' . $lesson_id : '') . ($assessment_id ? '/' . $assessment_id : ''), 'refresh');		
               // die("/c1/save/{$resource_id}/{$type}/{$parent_id}" . ($subject_id ? '/' . $subject_id : '') . ($module_id ? '/' . $module_id : '') . ($lesson_id ? '/' . $lesson_id : '') . ($assessment_id ? '/' . $assessment_id : '') );
           //   die(serialize($this->input->post('year_restriction')));
        }
	...
	private function resourceUpload()
		...
		//if ($this->upload->do_upload('fileupload'))
                ...
                $funm = explode('.', $_POST['fileupload_name']);
