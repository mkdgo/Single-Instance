...
class D5_teacher extends MY_Controller {
	...
	public function index($subject_id, $module_id, $lesson_id = '0') {
		...
		{	
			...
			$this->breadcrumbs->push($subject->name, "/d2_teacher/index/".$subject_id);
		}
		...
		if($lesson_id != 0) {
			if ($interactive_content_exists > 0) {
				$this->_data['create_edit_interactive_lesson'] = '<a href="/e1_teacher/index/'.$subject_id.'/'.$module_id.'/'.$lesson_id.'" style="margin:0 30px 0 20px;width:350px;float:right;" class="add_resource_butt black_button new_lesson_butt ui-link">EDIT INTERACTIVE LESSON</a>';
			} else {
				$this->_data['create_edit_interactive_lesson'] = '<a href="/e1_teacher/index/'.$subject_id.'/'.$module_id.'/'.$lesson_id.'" style="margin:0 30px 0 20px;width:350px;float:right;" class="add_resource_butt black_button new_lesson_butt ui-link">CREATE INTERACTIVE LESSON</a>';
			}
		} else {
