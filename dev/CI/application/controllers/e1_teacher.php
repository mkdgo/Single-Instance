...
class E1_teacher extends MY_Controller {
	...
	function index($subject_id = '',$module_id = '', $lesson_id = '') {
		...
		if(!empty($content_pages)){
			foreach ($content_pages as $kay => $val) {
				...
				$this->_data['content_pages'][$kay]['cont_page_id'] = $val->id;
                                ...
                                $ITEMS[]=Array('item_id'=>$val->id, 'item_type'=>"e2", 'item_type_delete'=>"delete", 'item_title'=>$val->title, 'item_order'=>$val->order);
			}
		}else{
		...
		foreach($int_assessments as $assessment_key =>$assessment){
			$this->_data['int_assessments'][$assessment_key]['assessment_id'] =$assessment->id;
                        ...
                        $ITEMS[]=Array('item_id'=>$assessment->id, 'item_type'=>"e3", 'item_type_delete'=>"delete_assessment", 'item_title'=>"Questions", 'item_order'=>$assessment->order);
		}
		}else{
	...
	function launch() {
		...
		$data = array(
			...
			'published_interactive_lesson' => $this->input->post('published_interactive_lesson'),
		);			
	...
	function save() {		
        ...
        {
            $slide_order_data = $this->input->post('resources_order');
