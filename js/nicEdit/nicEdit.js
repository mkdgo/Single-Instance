...
var nicEditorConfig = bkClass.extend({
	...
	buttonList : ['save','bold','italic','underline','left','center','align','justify','ol','ul','fontSize','fontFamily','fontFormat','indent','outdent','image','upload','link','unlink','forecolor','bgcolor'],
...
var nicEditor = bkClass.extend({
	...
	getIcon : function(iconName,options) {
		...
		return {backgroundImage : "url('"+((icon) ? this.options.iconsPath : file)+"')", backgroundPosition : ((icon) ? ((icon-1)*-38) : 0)+'px 0px'};	
	},
...
var nicEditorIFrameInstance = nicEditorInstance.extend({
	...
	init : function() {	
		...
		if(this.copyElm) { this.elmFrame.setStyle({width : (this.elm.offsetWidth-8)+'px'}); }
...
var nicEditorPanel = bkClass.extend({
	construct : function(e,options,nicEditor) {
		...
		this.panelElm = new bkElement('DIV').setStyle({margin : '4px', marginTop : '0px', zoom : 1, overflow : 'hidden'}).addClass('panel').appendTo(this.panelContain);
...
var nicEditorButton = bkClass.extend({
	...
	construct : function(e,buttonName,options,nicEditor) {
		...
		this.margin = new bkElement('DIV').setStyle({'float' : 'left', marginTop : '4px'}).appendTo(e);
...
//    case 'underline': btnName = "underline"; break;
            ...
            case 'justify': btnName = "align-justify"; break;
            case 'ol': btnName = "list-ol"; break;
            case 'ul': btnName = "list-ul"; break;
            case 'removeformat': btnName = "eraser"; break;
            case 'link': btnName = "chain"; break;
            case 'unlink': btnName = "chain-broken"; break;
...
var nicEditorAdvancedButton = nicEditorButton.extend({
	...
	addForm : function(f,elm) {
		...
		for(itm in f) {
			...
			if(type == 'title') {
					new bkElement('div').setContent(field.txt).setStyle({fontSize : '14px', fontWeight: 'bold', padding : '0px', margin : '8px 0'}).appendTo(this.form);
			} else {
				var contain = new bkElement('div').setStyle({overflow : 'hidden', clear : 'both'}).appendTo(this.form);
				if(field.txt) {
					new bkElement('label').setAttributes({'for' : itm}).setContent(field.txt).setStyle({margin : '8px 8px', fontSize : '13px', width: '50px', lineHeight : '40px', textAlign : 'right', 'float' : 'left'}).appendTo(contain);
				}
				
				switch(type) {
					case 'text':
						this.inputs[itm] = new bkElement('input').setAttributes({id : itm, 'value' : val, 'type' : 'text'}).setStyle({margin : '8px 0', fontSize : '13px', 'float' : 'left', height : '40px', border : '1px solid #ccc', overflow : 'hidden'}).setStyle(field.style).appendTo(contain);
						break;
					case 'select':
						this.inputs[itm] = new bkElement('select').setAttributes({id : itm}).setStyle({border : '1px solid #ccc', 'float' : 'left', margin : '8px 0'}).appendTo(contain);
...
var nicEditorSelect = bkClass.extend({
	...
	construct : function(e,buttonName,options,nicEditor) {
		...
		this.margin = new bkElement('div').setStyle({'float' : 'left', marginTop : '4px'}).appendTo(this.elm);
		this.contain = new bkElement('div').setStyle({width: '32px', height : '32px', cursor : 'pointer', overflow: 'hidden'}).addClass('selectContain').addEvent('click',this.toggle.closure(this)).appendTo(this.margin);
		this.items = new bkElement('div').setStyle({overflow : 'hidden', zoom : 1, paddingLeft : '3px'}).appendTo(this.contain);
		this.control = new bkElement('div').setStyle({overflow : 'hidden', 'float' : 'right', height: '30px', width : '30px', fontSize: '20px', textAlign: 'center' }).addClass('selectControl').appendTo(this.items);
		this.txt = new bkElement('div').setStyle({overflow : 'hidden', 'float' : 'left', width : '66px', height : '0px', marginTop : '0px', fontFamily : 'sans-serif', textAlign : 'center', fontSize : '12px'}).addClass('selectTxt').appendTo(this.items);
        this.control.addEvent('mouseover', this.hoverOn.closure(this)).addEvent('mouseout',this.hoverOff.closure(this));

        switch( buttonName ) {
            case 'fontSize': btnName = "text-height"; break;
            case 'fontFamily': btnName = "font"; break;
            case 'fontFormat': btnName = "header"; break;
            default : btnName = buttonName; break;
        }
        $(this.control).html('<i class="fa fa-'+btnName+'"></i>')
    ...
    hoverOn : function() {
        if(!this.isDisabled) {
            this.isHover = true;
            this.updateState();
            this.ne.fireEvent("buttonOver",this.pane);
        }
    }, 
    ...
    hoverOff : function() {
        this.isHover = false;
        this.updateState();
        this.ne.fireEvent("buttonOut",this.pane);
    },
    updateState : function() {
        if(this.isDisabled) { this.setBg(); }
        else if(this.isHover) { this.setBg('hover'); }
    ...
    setBg : function(state) {
        ...
        this.items.setStyle(stateStyle).addClass('pane-'+state);
    },
    

	over : function(opt) {
		opt.setStyle({backgroundColor : '#aaa'});			
	},
	
	out : function(opt) {
		opt.setStyle({backgroundColor : '#e9e9e9'});
	},
