...
function getArea(p, unique_n) {
    ...
    E = K = false;
    $.each(PG.items, function (key, val) {
        if (Number(val.unique_n) == Number(unique_n)) {
            E = val;
            K = key
        }
    });
...
function saveData() {
    ...
    global:
        ...
        $.each(PG.items, function (kd, vd) {
            if (vd.comment == "") {
                $($('#popupMessage').find('p')[0]).text('Please add a comment to `Comment ' + counter + '` box');
